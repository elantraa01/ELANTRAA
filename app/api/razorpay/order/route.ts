...
async function calculateOrderBreakdown(items: IncomingOrderItem[], promoCode: unknown) {
  ...
  // 40% of product amount + 100% of shipping charge paid in advance online
  const advanceProductAmount = Math.ceil(netProductAmount * 0.4);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If partial COD, customer pays 40% product value + shipping charge online
